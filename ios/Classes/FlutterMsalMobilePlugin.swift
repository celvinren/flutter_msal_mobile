...
public class FlutterMsalMobilePlugin: NSObject, FlutterPlugin {
  ...
  public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult)
    ...
    private func initialize(clientId: String, authority: String, authMiddleware: String, tenantType: String, loginHint: String, result: @escaping FlutterResult)
    {
        ...
        FlutterMsalMobilePlugin.clientId = clientId;
        FlutterMsalMobilePlugin.authority = authority;
        FlutterMsalMobilePlugin.authMiddleware = authMiddleware;
        FlutterMsalMobilePlugin.tenantType = tenantType;
        FlutterMsalMobilePlugin.loginHint = loginHint;
...
//MARK: - Get token
extension FlutterMsalMobilePlugin {
    ...
    private func acquireToken(scopes: [String], result: @escaping FlutterResult)
    {
        if let application = getApplication(result: result){
            ...
            if #available(iOS 13.0, *) {
                ...
                webViewParameters.webviewType = FlutterMsalMobilePlugin.authMiddleware != "webView" ? MSALWebviewType.safariViewController : MSALWebviewType.wkWebView
            }
            ...
            if (FlutterMsalMobilePlugin.loginHint != "") {
                interactiveParameters.loginHint = FlutterMsalMobilePlugin.loginHint
            }
...
extension FlutterMsalMobilePlugin {
    ...
    var currentAccountIdentifier: String? {
        get {
            return UserDefaults.standard.string(forKey: FlutterMsalMobilePlugin.kCurrentAccountIdentifier)
        }
        set (accountIdentifier) {
            ...
            // NSUserDefaults.
            UserDefaults.standard.set(accountIdentifier, forKey: FlutterMsalMobilePlugin.kCurrentAccountIdentifier)
        }
    }
    ...
    func clearCurrentAccount() {
        ...
        UserDefaults.standard.removeObject(forKey: FlutterMsalMobilePlugin.kCurrentAccountIdentifier)
    }
...
extension FlutterMsalMobilePlugin {
    ...
    {
        if(FlutterMsalMobilePlugin.clientId.isEmpty){
        ...
        if !FlutterMsalMobilePlugin.authority.isEmpty
        {
            //try creating the msal aad authority object
            do{
                //create authority url
                guard let authorityUrl = URL(string: FlutterMsalMobilePlugin.authority) else{
                ...
                switch FlutterMsalMobilePlugin.tenantType {
                    case "entraIDAndMicrosoftAccount":
                        let msalAuthority = try MSALAuthority(url: authorityUrl)
                        config = MSALPublicClientApplicationConfig(clientId: FlutterMsalMobilePlugin.clientId, redirectUri: nil, authority: msalAuthority)
                    case "azureADB2C":
                        let msalB2CAuthority = try MSALB2CAuthority(url: authorityUrl)
                        config = MSALPublicClientApplicationConfig(clientId: FlutterMsalMobilePlugin.clientId, redirectUri: nil, authority: msalB2CAuthority)
        ...
        {
            config = MSALPublicClientApplicationConfig(clientId: FlutterMsalMobilePlugin.clientId)
        }
