...
class FlutterMsalMobilePlugin: FlutterPlugin, ActivityAware, MethodChannel.MethodCallHandler {
    ...
    fun setMethodCallHandler(binding: FlutterPlugin.FlutterPluginBinding) {
        if (channel != null) {
    ...
    private fun initializeMsal(context: Context, configFile: File, result: MethodChannel.Result) {
        if (msalBase?.isClientInitialized() == true) {
            result.success(true)
            return
        }

        if (!configFile.exists()) {
            result.error("CONFIG_ERROR", "Config file does not exist", null)
            return
        }
